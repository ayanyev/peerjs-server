...
app._initializeWSS = function (server) {
    ...
    this._wss.on('connection', function (socket) {
        ...
        if (!id || !token || !key || !name) {
            socket.send(JSON.stringify({
                type: 'ERROR',
                payload: {msg: 'No name, id, token or key supplied to websocket server'}
            }));
            socket.close();
            return;
        }
            ...
            self._checkKey(key, ip, function (err) {
                if (!err) {
                    if (!self._clients[key][id]) {
                        self._clients[key][id] = {name: name, token: token, ip: ip};
                        self._ips[ip]++;
                        socket.send(JSON.stringify({type: 'OPEN'}));

                        for (var destination in self._clients[key]) {

                            if (destination) {
                                if (destination.id !== id && destination.socket) {
                                    this._log('new user alert - id:', id, 'name:', name);
                                    destination.socket.send(JSON.stringify({type: 'NEW_USER', src: id, dst: destination.id, payload: name}));
                                }
                            }
                        }
...
app._newUserAlert = function (key, id, name) {

    for (var destination in this._clients[key]) {
    ...
    this.get('/:key/peers', function (req, res, next) {
        ...
        if (self._clients[key]) {
            self._checkAllowsDiscovery(key, function (isAllowed) {
                if (isAllowed) {
                    var ids = Object.keys(self._clients[key]);
                    var peers = [];
                    for (var i in ids) {
                        peers.push(self._clients[key][ids[i]]["name"] + "_" + ids[i])
                    }
                    res.send(peers);
                } else {
                    res.sendStatus(401);
    ...
    if (destination) {
        try {
            ...
            if (destination.socket) {
                this._log('resending', type, 'from', src, 'to', dst);
                ...
                this._log('resending', type, 'from', src, 'to', dst, 'failed - neither socket no res available');
                throw "Peer dead";
            }
        } catch (e) {
            ...
            this._log('resending', type, 'from', src, 'to', dst, 'failed - peer disconnects without closing connections');
