            ...
            self._checkKey(key, ip, function (err) {
                if (!err) {
                    if (!self._clients[key][id]) {
                        ...
                        // app._alertEverybody(key, id, name, 'NEW_USER');

                        for (var destination in self._clients[key]) {

                            if (destination !== id) {
                                var obj = self._clients[key][destination];
...
app._alertEverybody = function(key, id, name, type) {
...
app._removePeer = function (key, id) {
    if (this._clients[key] && this._clients[key][id]) {
        ...
        for (var destination in this._clients[key]) {
            ...
            if (destination !== id) {
                var obj = this._clients[key][destination];
                obj.socket.send(JSON.stringify({
                    type: 'LEAVE',
                    src: id,
                    dst: destination,
